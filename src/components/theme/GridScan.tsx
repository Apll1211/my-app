...
export const GridScan: React.FC<GridScanProps> = ({
	...
	const pushScan = useCallback((t: number) => {
		...
		if (materialRef.current) {
			...
			for (let i = 0; i < arr.length && i < 8; i++) buf[i] = arr[i];
		...
		const onClick = async () => {
			...
			if (
				...
				try {
					...
					// Handle permission request error silently
				}
			}
		};
		const onEnter = () => {
	...
	}, [uiFaceActive, snapBackDelay, scanOnClick, enableGyro, pushScan]);
	...
	useEffect(() => {
		...
		const tick = () => {
			...
			const colorPhase = (now / 1000) % 12;
			const hueRotation = (colorPhase / 12) * 360;
			const brightness = 1.8 + Math.sin((colorPhase * Math.PI * 2) / 12) * 0.4;
			const saturationBoost =
				1.3 + Math.sin((colorPhase * Math.PI * 2) / 12) * 0.3;

			const baseScanColor = new THREE.Color(scanColor);
			const shiftedColor = baseScanColor.clone();
			const hsl = { h: 0, s: 0, l: 0 };
			shiftedColor.getHSL(hsl);
			const adjustedBrightness = Math.max(
				0.3,
				Math.min(0.7, hsl.l * brightness),
			);
			shiftedColor.setHSL(
				(hsl.h + hueRotation / 360) % 1,
				Math.min(1, hsl.s * saturationBoost),
				adjustedBrightness,
			);

			material.uniforms.uScanColor.value = shiftedColor;
	...
	useEffect(() => {
		...
		if (bloomRef.current) {
			...
			const bloomAny = bloomRef.current as unknown as {
				luminanceMaterial: {
					threshold: number;
					smoothing: number;
				};
			};
			bloomAny.luminanceMaterial.threshold = bloomThreshold;
			bloomAny.luminanceMaterial.smoothing = bloomSmoothing;
		}
